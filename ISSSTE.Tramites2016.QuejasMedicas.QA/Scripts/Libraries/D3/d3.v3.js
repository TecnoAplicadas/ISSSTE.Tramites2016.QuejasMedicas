d3 = function() {
    ...
    function d3_mousePoint(container, e) {
        ...
        if (svg.createSVGPoint) {
            ...
            if (d3_mouse_bug44083 < 0 && (d3_window.scrollX || d3_window.scrollY)) {
                svg = d3.select(d3_document.body).append("svg").style("position", "absolute").style("top", 0)
                    .style("left", 0);
    ...
    d3.behavior.drag = function() {
        ...
        function mousedown() {
            var target = this,
                ...
                origin_ = point(),
                moved = 0;
            var w = d3.select(d3_window).on(touchId != null ? "touchmove.drag-" + touchId : "mousemove.drag", dragmove)
    ...
    d3.behavior.zoom = function() {
        ...
        function zoom() {
            this.on("mousedown.zoom", mousedown).on("mousemove.zoom", mousemove)
                .on(d3_behavior_zoomWheel + ".zoom", mousewheel).on("dblclick.zoom", dblclick)
    ...
    d3_rgbPrototype.brighter = function(k) {
        ...
        if (g && g < i) g = i;
        if (b && b < i) b = i;
        return d3_rgb(Math.min(255, Math.floor(r / k)),
    ...
    d3.xhr = function(url, mimeType, callback) {
        var xhr = {},
            ...
            request =
                new (d3_window.XDomainRequest && /^(http(s)?:)?\/\//.test(url) ? XDomainRequest : XMLHttpRequest)();
        "onload" in request
    ...
    d3.geo.centroid = function(object) {
        ...
        var m;
        if (d3_geo_centroidW &&
            Math.abs(m = Math.sqrt(d3_geo_centroidX * d3_geo_centroidX +
    ...
    function d3_geo_centroidLineStart() {
        ...
        function nextPoint(λ, φ) {
            ...
            var cosφ = Math.cos(φ *= d3_radians),
                ...
                y = cosφ * Math.sin(λ),
                z = Math.sin(φ),
                w = Math.atan2(Math.sqrt((w = y0 * z - z0 * y) * w +
                        (w = z0 * x - x0 * z) * w +
                        (w = x0 * y - y0 * x) * w),
                    x0 * x + y0 * y + z0 * z);
            d3_geo_centroidW += w;
    ...
    d3.geo.albersUsa = function() {
        ...
        albersUsa.invert = function(coordinates) {
            return alaskaInvert(coordinates) ||
    ...
    d3.geo.path = function() {
        ...
        function path(object) {
            if (object)
                d3.geo.stream(object,
                    projectStream(contextStream.pointRadius(typeof pointRadius === "function"
        ...
        path.centroid = function(object) {
            ...
            return d3_geo_centroidZ
                ? [d3_geo_centroidX / d3_geo_centroidZ, d3_geo_centroidY / d3_geo_centroidZ]
                : undefined;
        };
    ...
    function d3_svg_lineCardinalOpen(points, tension) {
        return points.length < 4
            ...
            : points[1] +
            d3_svg_lineHermite(points.slice(1, points.length - 1), d3_svg_lineCardinalTangents(points, tension));
    }
    ...
    d3.geom.delaunay = function(vertices) {
        var edges = vertices.map(function() {
            ...
            triangles = [];
        d3_geom_voronoiTessellate(vertices,
            function(e) {
    ...
    d3.geom.voronoi = function(points) {
        ...
        function voronoi(data) {
            ...
            d3_geom_voronoiTessellate(points,
                function(e) {
    ...
    d3.layout.pie = function() {
        ...
        function pie(data) {
            ...
            var k = ((typeof endAngle === "function" ? endAngle.apply(this, arguments) : endAngle) - a) /
                d3.sum(values);
    ...
    d3_transitionPrototype.attr = function(nameNS, value) {
        ...
        return d3_transition_tween(this,
            "attr." + nameNS,
            value,
            function(b) {
                function attrString() {
        ...
        function axis(g) {
            g.each(function() {
                ...
                var subticks = d3_svg_axisSubdivide(scale, ticks, tickSubdivide),
                    ...
                    subtickEnter =
                        subtick.enter().insert("line", ".tick").attr("class", "tick minor").style("opacity", 1e-6),
    ...
    d3.svg.brush = function() {
        ...
        function brushstart() {
            var target = this,
                ...
                origin = mouse(),
                offset;
            var w = d3.select(d3_window).on("mousemove.brush", brushmove).on("mouseup.brush", brushend)
                .on("touchmove.brush", brushmove).on("touchend.brush", brushend).on("keydown.brush", keydown)
    ...
    var d3_time_scaleLocalMethods = [
        [d3.time.second, 1], [d3.time.second, 5], [d3.time.second, 15], [d3.time.second, 30], [d3.time.minute, 1],
        [d3.time.minute, 5], [d3.time.minute, 15], [d3.time.minute, 30], [d3.time.hour, 1], [d3.time.hour, 3],
        [d3.time.hour, 6], [d3.time.hour, 12], [d3.time.day, 1], [d3.time.day, 2], [d3.time.week, 1],
        [d3.time.month, 1], [d3.time.month, 3], [d3.time.year, 1]
    ];
