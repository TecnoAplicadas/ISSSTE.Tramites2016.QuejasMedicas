                ...
                function() {
                    var elementStateModifiers = {},
                        ...
                        invalidElementStylingEnabled = function(el) {
                            return enableInvalidElementStyling &&
                                !getBooleanAttributeValue(el, "disable-invalid-styling");
                        };
                    ...
                    this.getErrorMessage = function(errorKey, el) {
                        ...
                        if (this.errorMessageResolver === undefined) {
                            throw new Error(
                                "Please set an error message resolver via the setErrorMessageResolver function before attempting to resolve an error message.");
                        }
                    ...
                    this.getDomModifier = function(el) {
                        var modifierKey =
                            (el !== undefined ? el.attr("element-modifier") : this.defaultElementModifier) ||
                                (el !== undefined ? el.attr("data-element-modifier") : this.defaultElementModifier) ||
                                this.defaultElementModifier;

                        if (modifierKey === undefined) {
                            throw new Error(
                                "Please set a default dom modifier via the setDefaultElementModifier method on the validator class.");
                        }
            ...
            [
                ...
                function($log) {
                    var reset = function(el) {
                            angular.forEach(el.find("small"),
                                function(spanEl) {
                                    spanEl = angular.element(spanEl);
                                    if (spanEl.hasClass("error-msg") ||
                                        spanEl.hasClass("form-control-feedback") ||
                                        spanEl.hasClass("control-feedback")) {
                                        spanEl.remove();
                                    }
                                });
                        ...
                        makeValid = function(el) {
                            ...
                            if (frmGroupEl) {
                                ...
                                frmGroupEl.addClass((inputGroupEl.length > 0 ? "" : "has-feedback"));
                                if (addValidationStateIcons) {
                                    var iconElText =
                                        '<small class="glyphicon glyphicon-ok form-control-feedback"></small>';
                                    if (inputGroupEl.length > 0) {
                                        iconElText = iconElText.replace("form-", "");
                                        iconElText = '<small class="input-group-addon control-feedback">' +
                                            iconElText +
                                            "</small";
                                    }
                        ...
                        makeInvalid = function(el, errorMsg) {
                            var frmGroupEl = findFormGroupElement(el),
                                helpTextEl = angular.element('<small class="help-block has-error error-msg">' +
                                    errorMsg +
                                    "</small>"),
                                inputGroupEl;
                            ...
                            if (frmGroupEl) {
                                ...
                                if (addValidationStateIcons) {
                                    var iconElText =
                                        '<small class="glyphicon glyphicon-remove form-control-feedback"></small>';
                                    if (inputGroupEl.length > 0) {
                                        iconElText = iconElText.replace("form-", "");
                                        iconElText = '<small class="input-group-addon control-feedback">' +
                                            iconElText +
        ...
        [
            ...
            function($timeout) {
                var debounce = function(fn, timeout, apply) {
                    ...
                    apply =
                        angular.isUndefined(apply) ? true : apply; // !!default is true! most suitable to my experience
                    var nthCall = 0;
...
(function(String, angular) {
    ...
    angular.autoValidate.errorMessages["en-us"] = angular.autoValidate.errorMessages["en-gb"] = {
        defaultMsg: "Ha ocurrido un error para {0}",
        email: "Ingresa un correo v&aacute;lido",
        minlength: "Ingresa al menos  {0} caracteres",
        maxlength: "Debes ingresar  {0} caracteres",
        min: "Ingresa al menos {0} caracteres ",
        max: "Ingresa  {0} caracteres ",
        required: "Este campo es requerido",
        date: "Ingresa una fecha v&aacute;lida",
        pattern: "Verifica que la informaci&oacute;n ingresada cumpla este patron {0}",
        number: "Ingresa s&oacute;lo n&uacute;meros",
        url: "Ingresa una URL valida como la siguiente http(s)://www.google.com",
        postalcodeM: "Ingresar CP v&aacute;lido"
    };
                ...
                function($q, $http) {
                    var currentCulture = "en-gb",
                        ...
                        loadRemoteCulture = function(culture) {
                            cultureRetrievalPromise =
                                $http.get(
                                    "{0}/jcs-auto-validate_{1}.json".format(i18nFileRootPath, culture.toLowerCase()));
                        ...
                        resolve = function(errorType, el) {
                            ...
                            if (cultureRetrievalPromise !== undefined) {
                                ...
                                if (errMsg === undefined) {
                                    errMsg =
                                        angular.autoValidate.errorMessages[currentCulture].defaultMsg.format(errorType);
                                }
                ...
                function(validator, elementUtils) {
                    var elementTypesToValidate = ["input", "textarea", "select", "form"],
                        ...
                        getFormOptions = function(el) {
                            ...
                            return frmCtrl !== undefined && frmCtrl !== null
                                ? frmCtrl.autoValidateFormOptions
                                : validator.defaultFormValidationOptions;
                        },
                        ...
                        validateElement = function(modelCtrl, el, options) {
                            ...
                            if (frmOptions.disabled === false) {
                                if ((frmOptions.forceValidation ||
                                    (shouldValidateElement(el, frmOptions) && modelCtrl && needsValidation))) {
                                    isValid = !modelCtrl.$invalid;

                                    if (frmOptions.removeExternalValidationErrorsOnSubmit &&
                        ...
                        resetForm = function(frmElement) {
                            angular.forEach((frmElement[0].all || frmElement[0].elements) || frmElement[0],
                                function(element) {
                                    var controller,
                        ...
                        validateForm = function(frmElement) {
                            var frmValid = true,
                                ...
                                processElement = function(ctrlElement, force, formOptions) {
                                    ...
                                    if (controller !== undefined &&
                                        (force || shouldValidateElement(ctrlElement, formOptions))) {
                                        if (ctrlElement[0].nodeName.toLowerCase() === "form") {
                                            // we probably have a sub form
                            ...
                            if (frmElement === undefined ||
                                (frmCtrl !== undefined && frmCtrl.autoValidateFormOptions.disabled)) {
                            ...
                            if (frmElement[0].customHTMLFormControlsCollection) {
                                angular.forEach(frmElement[0].customHTMLFormControlsCollection,
                                    function(ctrlElement) {
...
(function(angular) {
    ...
    function parseOptions(ctrl, validator, attrs) {
        var opts = ctrl.autoValidateFormOptions = ctrl.autoValidateFormOptions ||
            angular.copy(validator.defaultFormValidationOptions);
        ...
        opts.removeExternalValidationErrorsOnSubmit = attrs.removeExternalValidationErrorsOnSubmit === undefined
            ? true
            : parseBooleanAttributeValue(attrs.removeExternalValidationErrorsOnSubmit);
                        ...
                        directive.compile = function(el) {
                            return function(scope, element, attrs, ctrls) {
                                var ngModelCtrl = ctrls[0],
                                    ...
                                    ngModelOptions = attrs.ngModelOptions === undefined
                                        ? undefined
                                    ...
                                    setValidationState = debounce.debounce(function() {
                                            var validateOptions =
                                                frmCtrl !== undefined && frmCtrl !== null
                                                    ? frmCtrl.autoValidateFormOptions
                                                    : undefined;
                                ...
                                if (link.pre) {
                                    ...
                                    ngModelOptions = ngModelCtrl.$options === undefined
                                        ? undefined
                                        : ngModelCtrl.$options;
                                }
                                ...
                                (frmCtrl !== undefined &&
                                    ...
                                    if (supportsNgModelOptions ||
                                        ngModelOptions === undefined ||
                                        ngModelOptions.updateOn === undefined ||
                                        ...
                                        element.on(ngModelOptions.updateOn,
                                            function() {
                                                setValidationState();
                                            });
                                ...
                                ngModelCtrl.setExternalValidation =
                                    function(errorMsgKey, errorMessage, addToModelErrors) {
                                        if (addToModelErrors) {
                                ...
                                ngModelCtrl.removeAllExternalValidation = function() {
                                    if (ngModelCtrl.externalErrors) {
                                        ...
                                        angular.forEach(ngModelCtrl.externalErrors,
                                            function(value, key) {
                                ...
                                if (frmCtrl) {
                                    frmCtrl.setExternalValidation =
                                        function(modelProperty, errorMsgKey, errorMessageOverride, addToModelErrors) {
                                            var success = false;
                                            if (frmCtrl[modelProperty]) {
                                                frmCtrl[modelProperty].setExternalValidation(errorMsgKey,
                                    ...
                                    frmCtrl.removeExternalValidation =
                                        function(modelProperty, errorMsgKey, errorMessageOverride, addToModelErrors) {
                                            var success = false;
                                            if (frmCtrl[modelProperty]) {
                                                frmCtrl[modelProperty].removeExternalValidation(errorMsgKey,
                                                    addToModelErrors);
                                                success = true;
                                            }
