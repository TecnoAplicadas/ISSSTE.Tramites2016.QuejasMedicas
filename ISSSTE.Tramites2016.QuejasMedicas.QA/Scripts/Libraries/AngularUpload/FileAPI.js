...
(function(window) {
    ...
    var CanvasPrototype = window.HTMLCanvasElement &&
        ...
        dataURLtoBlob = (hasBlobConstructor || BlobBuilder) &&
            window.atob &&
            window.ArrayBuffer &&
            window.Uint8Array &&
            function(dataURI) {
...
(function(window, undef) {
    ...
    var
        ...
        html5 =
            !!(File && (FileReader && (window.Uint8Array || FormData || XMLHttpRequest.prototype.sendAsBinary))) &&
                !(/safari\//i.test(userAgent) &&
                    !/chrome\//i.test(userAgent) &&
                    /windows/i.test(userAgent)), // BugFix: https://github.com/mailru/FileAPI/issues/25
        ...
        api = {
            ...
            getFilesFilter: function(filter) {
                filter = typeof filter == "string"
                    ? filter
                    : (filter.getAttribute && filter.getAttribute("accept") || "");
            ...
            getDropFiles: function(evt, callback) {
                var
                    ...
                    entrySupport =
                        _isArray(dataTransfer.items) && dataTransfer.items[0] && _getAsEntry(dataTransfer.items[0]),
                    queue = api.queue(function() { callback(files); });

                _each((entrySupport ? dataTransfer.items : dataTransfer.files) || [],
                    function(item) {
                        queue.inc();

                        try {
                            if (entrySupport) {
                                _readEntryAsFiles(item,
                                    function(err, entryFiles) {
                                        if (err) {
            ...
            upload: function(options) {
                ...
                _nextFile = function() {
                    ...
                    if ((proxyXHR.statusText != "abort" || proxyXHR.current) && data) {
                        ...
                        _this._getFormData(_fileOptions,
                            data,
                            function(form) {
                                if (!_loaded) {
                                    // emit "upload" event
                                            ...
                                            ? function(evt) {
                                                if (!_fileLoaded) {
                                                    ...
                                                    options.progress({
                                                            type: "progress",
                                                            total: _total,
                                                            loaded: proxyXHR.loaded =
                                                                (_loaded + data.size * (evt.loaded / evt.total)) | 0
                                                        },
                                                        _file,
                                                        xhr,
                                                        _fileOptions);
                                        ...
                                        complete: function(err) {
                                            ...
                                            if (_file) {
                                                ...
                                                if (!err) {
                                                    ...
                                                    // bytes loaded
                                                    _loaded +=
                                                        data.size; // data.size != data.total, it's desirable fix this
            ...
            _getFormData: function(options, data, fn) {
                ...
                (function _addFile(file /**Object*/) {
                    if (file.image) { // This is a FileAPI.Image
                        ...
                        api.Image.transform(file,
                            trans,
                            options.imageAutoOrientation,
                            function(err, images) {
                                if (isOrignTrans && !err) {
                                    ...
                                    if (!err) {
                                        _each(images,
                                            function(image, idx) {
                                                ...
                                                Form.append(trans[idx].postName || postNameConcat(name, idx),
                                                    image,
                                                    filename,
                                                    trans[idx].type || filetype);
                                            });
                                    }

                                    if (err || options.imageOriginal) {
                                        Form.append(postNameConcat(name, (addOrigin ? "original" : null)),
                                            file,
                                            filename,
                                            filetype);
                                    }
                        ...
                        if (xhr.readyState == 4) {
                            ...
                            if (xhr.status == 200) {
                                ...
                                file.dataURL = "data:" +
                                    file.type +
                                    ";base64," +
                                    api.encode64(xhr.responseBody || xhr.responseText);
    ...
    api.addInfoReader(/^image/,
        function(file /**File*/, callback /**Function*/) {
            if (!file.__dimensions) {
                var defer = file.__dimensions = api.defer();

                api.readAsImage(file,
                    function(evt) {
                        var img = evt.target;
    ...
    api.event.dnd = function(el, onHover, onDrop) {
        ...
        if (FileReader) {
            ...
            _on(el,
                ...
                onHover.ff = onHover.ff ||
                function(evt) {
                    var
                        types = _getDataTransfer(evt).types, i = types && types.length, debounceTrigger = false;
    ...
    _each(document.getElementsByTagName("meta"),
        function(meta) {
...
(function(api, document, undef) {
    ...
    Image.prototype = {
        ...
        _apply: function(image, fn) {
            ...
            renderImageToCanvas(ctx.canvas,
                buffer,
                m.sx,
                m.sy,
                m.sw || buffer.width,
                    ...
                    function(params, name) {
                        if (!queue.isFail()) {
                            ...
                            if (isFn) {
                                ...
                                ImgTrans[params.preview ? "preview" : "resize"](params.width,
                                    params.height,
                                    params.strategy);
                            } else {
    ...
    // @const
    api.each(["TOP", "CENTER", "BOTTOM"],
        function(x, i) {
            api.each(["LEFT", "CENTER", "RIGHT"],
                function(y, j) {
                    Image[x + "_" + y] = i * 3 + j;
                    Image[y + "_" + x] = i * 3 + j;
                });
    ...
    Image.applyFilter = function(canvas, filter, doneFn) {
        if (typeof filter == "function") {
            ...
            window.Caman(canvas.tagName == "IMG" ? Image.toCanvas(canvas) : canvas,
                function() {
                    if (typeof filter == "string") {
                        this[filter]();
                    } else {
...
(function(api, window) {
    ...
    Form.prototype = {
        ...
        toData: function(fn, options) {
            ...
            options._chunked = api.support.chunked &&
                options.chunkSize > 0 &&
                api.filter(this.items, function(item) { return item.file; }).length == 1;
        ...
        toHtmlData: function(fn) {
            this._to(document.createDocumentFragment(),
                fn,
                function(file, data /**DocumentFragment*/) {
        ...
        toMultipartData: function(fn) {
            this._to([],
                fn,
                function(file, data, queue, boundary) {
                    queue.inc();
                    _convertFile(file,
                        function(file, blob) {
                            data.push(
                                ...
                                ('\r\nContent-Disposition: form-data; name="' +
                                    file.name +
                                    '"' +
                                    (file.file ? '; filename="' + encodeURIComponent(file.file) + '"' : "") +
                                    (file.file
    ...
    XHR.prototype = {
        ...
        _send: function(options, data) {
            ...
            if (data.nodeName) {
                ...
                xhr.innerHTML = '<form target="' +
                    uid +
                    '" action="' +
                    url +
                    '" method="POST" enctype="multipart/form-data" style="position: absolute; top: -1000px; overflow: hidden; width: 1px; height: 1px;">' +
                    '<iframe name="' +
                    uid +
                    '" src="javascript:false;" onload="' +
                    onloadFuncName +
                    '()"></iframe>' +
                    (jsonp && (options.url.indexOf("=?") < 0)
                        ? '<input value="' + uid + '" name="' + jsonp + '" type="hidden"/>'
                        : "") +
                    "</form>";
                ...
                if (options._chunked) {
                    ...
                    if (xhr.upload) {
                        xhr.upload.addEventListener("progress",
                            api.throttle(function(/**Event*/evt) {
                                    if (!data.retry) {
                    ...
                    xhr.onreadystatechange = function() {
                        ...
                        if (xhr.readyState == 4) {
                            ...
                            if (!xhr.status || xhr.status - 201 > 0) {
                                ...
                                // up - server error
                                if (((!xhr.status && !xhr.aborted) || 500 == xhr.status || 416 == xhr.status) &&
                                    ++data.retry <= options.chunkUploadRetry) {
                    ...
                    xhr.onreadystatechange = function() {
                        ...
                        if (xhr.readyState == 4) {
                            ...
                            if (!xhr.status || xhr.status > 201) {
                                api.log("Error: " + xhr.status);
                                if (((!xhr.status && !xhr.aborted) || 500 == xhr.status) &&
                                    (options.retry || 0) < options.uploadRetry) {
                    ...
                    if (api.isArray(data)) {
                        ...
                        if (xhr.sendAsBinary) {
                            ...
                            var bytes =
                                Array.prototype.map.call(rawData, function(c) { return c.charCodeAt(0) & 0xff; });
...
(function(window, jQuery, api) {
    ...
    var
        document = window.document, location = window.location, navigator = window.navigator, _each = api.each;
    ...
    api.support.flash = (function() {
        ...
        if (navigator.plugins && typeof navigator.plugins["Shockwave Flash"] == "object") {
            has = navigator.plugins["Shockwave Flash"].description &&
                !(mime &&
                    mime["application/x-shockwave-flash"] &&
                    !mime["application/x-shockwave-flash"].enabledPlugin);
        } else {
            try {
    ...
    api.support.flash &&
        (0 || !api.html5 || !api.support.html5 || (api.cors && !api.support.cors) || (api.media && !api.support.media)
        ) &&
        (function() {
            var
                _attr = api.uid(),
...
//						, src: _getUrl('http://v.demidov.boom.corp.mail.ru/uploaderfileapi/FlashFileAPI.swf?1')
                                ...
                                flashvars: "callback=" +
                                    (opts.onEvent || "FileAPI.Flash.onEvent") +
                                    "&flashId=" +
                                    id +
                                    "&storeKey=" +
                                    navigator.userAgent.match(/\d/ig).join("") +
                                    "_" +
                                    api.version +
                                    (flash.isReady || (api.pingUrl ? "&ping=" + api.pingUrl : "")) +
                                    "&timeout=" +
                                    api.flashAbortTimeout +
                    ...
                    mouseenter: function(evt) {
                        ...
                        if (node) {
                            ...
                            _each((node.getAttribute("accept") || "").split(/,\s*/),
                                function(mime) {
                                    api.accept[mime] &&
                                        _each(api.accept[mime].split(" "),
                                            function(ext) {
                                                exts[ext] = 1;
                                            });
                                });
                            ...
                            flash.cmd(evt,
                                "accept",
                                accept.length ? accept.join(",") + "," + accept.join(",").toUpperCase() : "*");
                        }
                    },
                    ...
                    select: function(evt) {
                        try {
                            var
                                inp = flash.getInput(evt.flashId), uid = api.uid(inp), files = evt.target.files, event;
                            _each(files,
                                function(file) {
                        ...
                        _inherit(api,
                            {
                                ...
                                getFiles: function(input, filter, callback) {
                                    ...
                                    var files = api.isArray(input)
                                        ? input
                                        : _files[api.uid(input.target || input.srcElement || input)];
...
//									});
                            ...
                            _inherit(api.Image.prototype,
                                {
                                            ...
                                            flash.cmd(file,
                                                "imageTransform",
                                                {
                                                    ...
                                                    callback: _wrap(function _(err, base64) {
                                                        ...
                                                        if (err) {
                                                            doneFn(err);
                                                        } else if (!api.support.html5 &&
                                                            (!api.support.dataURI || base64.length > 3e4)) {
                                                            _makeFlashImage({
                                                            ...
                                                            api.newImage("data:" + file.type + ";base64," + base64,
                                                                doneFn);
                                                        }
                                                    })
                                                });
                                    ...
                                    toData: function(fn) {
                                        var
                                            file = this.file, info = file.info, matrix = this.getMatrix(info);
                                        ...
                                        if (_isHtmlFile(file)) {
                                            ...
                                            if (matrix.deg == "auto") {
                                                matrix.deg =
                                                    api.Image.exifOrientation[info &&
                                                        info.exif &&
                        ...
                        _inherit(api.XHR.prototype,
                            {
                                _send: function(options, formData) {
                                    ...
                                    var
                                        data = {}, files = {}, _this = this, flashId, fileId;
                                                ...
                                                callback: _wrap(function upload(evt) {
                                                    ...
                                                    if (type == "progress") {
                                                        ...
                                                        if (typeof result == "string") {
                                                            _this.responseText =
                                                                result.replace(/%22/g, "\"").replace(/%5c/g, "\\")
                                                                .replace(/%26/g, "&").replace(/%25/g, "%");
                                                        }
            ...
            function _makeFlashHTML(opts) {
                return ('<object id="#id#" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="' +
                    ...
                    '<embed flashvars="#flashvars#" swliveconnect="true" allownetworking="all" allowscriptaccess="always" name="#id#" src="#src#" width="' +
                    (opts.width || "100%") +
                    '" height="' +
                    (opts.height || "100%") +
                    '" menu="false" wmode="transparent" type="application/x-shockwave-flash"></embed>' +
            ...
            function _makeFlashImage(opts, base64, fn) {
                var
                    key, flashId = api.uid(), el = document.createElement("div"), attempts = 10;
            ...
            api.newImage("data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw==",
                function(err, img) {
