(function() {
    ...
    var usetemplateUrl = "template.html";
    ...
    function ngDuallist($rootScope, $filter, $parse) {
        ...
        var directive = {
            ...
            templateUrl: function(element, attrs) {
                return attrs.templateUrl || usetemplateUrl;
            },
            replace: true
        };
        return directive;
        ...
        function link(scope, element, attrs, controller) {
            ...
            if (scope.duallistOption.leftContainerScrollEnd) { // excecute the left container scroll end event
                $(element).find(".left-sub-container").bind("scroll",
                    function() {
            ...
            if (scope.duallistOption.rightContainerScrollEnd) { // execute the righ contained scroll end event
                $(element).find(".right-sub-container").bind("scroll",
                    function() {
                        if ($(this)[0].scrollHeight - $(this).scrollTop() === $(this).height()) {
            ...
            if (scope.duallistOption.leftContainerSearch) { //left seracg text chage
                $(element).find("#leftsearch").bind("change paste keyup",
                    function() {
                        var value = $(this).val();
            ...
            if (scope.duallistOption.rightContainerSearch) { //right serach text chage
                $(element).find("#rightsearch").bind("change paste keyup",
                    function() {
                        var value = $(this).val();
            ...
            if (scope.duallistOption.onMoveRight) { //right serach text chage
                $(element).find("#moveRightButton").bind("click",
                    function() {
            ...
            if (scope.duallistOption.onMoveLeft) { //right serach text chage
                $(element).find("#moveLeftButton").bind("click",
                    function() {
    ...
    function Controller($scope) {
        ...
        $scope.selectAllLeftContainer = function() {
            ...
            angular.forEach($scope.leftscope,
                function(val) {
        ...
        $scope.selectAllRightContainer = function() {
            angular.forEach($scope.rightscope,
                function(val) {
    ...
    function templateCache($templateCache) {
        $templateCache.put(usetemplateUrl,
            '<div class="row ngduallist"><div class="col-sm-4 left-container"><label>{{duallistOption.leftContainerLabel}}</label><span class=info-container><span class=info ng-if="leftscope.length > 0">{{leftscope.length}} Elemento(s)</span> <span class=info ng-if="leftscope.length == 0">Lista vacia</span></span><form class=form-inline><div><div class=input-group><input class=form-control id=leftsearch placeholder=Search><div class=input-group-addon><input title="toggle all" class="all pull-right" ng-model=leftSelectAll ng-change=selectAllLeftContainer() type=checkbox></div></div></div></form><div class=left-sub-container><div class=list-group id=list1><div ng-class="{active:data.selected}" ng-repeat="data in leftscope track by $index" class=list-group-item>{{data.name}} <input class=pull-right ng-model=data.selected type=checkbox></div></div></div></div><div class="col-sm-1 v-center"><button id=moveRightButton ng-click=moveRight() title="Send to list 2" class="btn btn-default center-block add"><i class="glyphicon glyphicon-chevron-right"></i></button> <button id=moveLeftButton ng-click=moveLeft() title="Send to list 1" class="btn btn-default center-block remove"><i class="glyphicon glyphicon-chevron-left"></i></button></div><div class="col-sm-4 right-container"><label>{{duallistOption.rightContainerLabel}}</label><span class=info-container><span class=info ng-if="rightscope.length > 0">{{rightscope.length}} Elemento(s)</span> <span class=info ng-if="rightscope.length == 0">Lista vacia</span></span><form class=form-inline><div><div class=input-group><input class=form-control id=rightsearch placeholder=Search><div class=input-group-addon><input ng-model=rightSelectAll ng-change=selectAllRightContainer() title="toggle all" class="all pull-right" type=checkbox></div></div></div></form><div class=right-sub-container><div class=list-group id=list2><div ng-class="{active:data.selected}" ng-repeat="data in rightscope track by $index" class=list-group-item>{{data.name}} <input ng-model=data.selected class=pull-right type=checkbox></div></div></div></div></div>');
