                        ...
                        var config = {
                            ...
                            complete: function(err, fileApiXHR) {
                                ...
                                var resp = fileApiXHR.responseText ||
                                    (err && fileApiXHR.status === 0 && err !== "abort" ? err : undefined);
                                redefineProp(xhr,
                                    "responseText",
                                    function() {
                                        return resp;
        ...
        "$http", "$q", "$timeout", function($http, $q, $timeout) {
            ...
            this.upload = function(config) {
                ...
                config.transformRequest = config.transformRequest
                    ? (angular.isArray(config.transformRequest) ? config.transformRequest : [config.transformRequest])
                    : [];
...
(function() {
    ...
    function linkFileSelect(scope, elem, attr, ngModel, $parse, $timeout, $compile) {
        ...
        if (getAttr(attr, "ngfSelect").search(/\W+$files\W+/) === -1) {
            scope.$watch(getAttr(attr, "ngfSelect"),
                function(val) {
                    disabled = val === false;
                });
        }
            ...
            if (!isUpdating) {
                ...
                try {
                    ...
                    updateModel($parse,
                        ...
                        ngModel,
                        attr,
                        getAttr(attr, "ngfChange") || getAttr(attr, "ngfSelect"),
                        files,
                        rejFiles,
                        evt);
    ...
    ngFileUpload.updateModel = function($parse,
        $timeout,
        scope,
        ngModel,
        attr,
...
(function() {
    ...
    ngFileUpload.directive("ngfNoFileDrop",
        function() {
    ...
    function linkDrop(scope, elem, attr, ngModel, $parse, $timeout, $location) {
        ...
        if (getAttr(attr, "ngfDrop").search(/\W+$files\W+/) === -1) {
            scope.$watch(getAttr(attr, "ngfDrop"),
                function(val) {
                    disabled = val === false;
                });
        }
                ...
                extractFiles(evt,
                    function(files, rejFiles) {
                        updateModel($parse,
                            ...
                            ngModel,
                            attr,
                            getAttr(attr, "ngfChange") || getAttr(attr, "ngfDrop"),
                            files,
                            rejFiles,
                            evt);
                    },
