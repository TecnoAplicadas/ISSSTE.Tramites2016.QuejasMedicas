    ...
    var EasyTree = function(jQueryContext, options) {
        ...
        this.init = function(jQueryContext, options) {
            ...
            if (_settings.dataUrl) {
                ajaxService(_settings.dataUrl,
                    _settings.dataUrlJson,
                    function(data) {
        ...
        function nodeClick(event) {
            ...
            var idPadre = node.idPadre == null ? null : node.idPadre;

            if (_settings.clickNode != null &&
                _settings.clickNode != undefined &&
                (idPadre != null || (node.href != "" && node.href != null))) {
                try {
                    _settings.clickNode();
                } catch (ex) {
                    //alert("clickNode no es una funcion");
                }
            }
        ...
        function sort(nodes) {
            ...
            nodes = nodes.sort(function(n1, n2) {
                ...
                if (_settings.ordering.toLowerCase().indexOf("folder") > -1 && n1.esCarpeta) {
                    n1Text = "______" + n1Text;
                }
                if (_settings.ordering.toLowerCase().indexOf("folder") > -1 && n2.esCarpeta) {
        ...
        function getNodesAsHtml(nodes, level, display) {
            ...
            for (i = 0; i < nodes.length; i++) {
                ...
                color = "#daf7a6";
                auxiliar = ' style="color:' + color + ';" ';

                html += "<li>";
                html += '<span id="' + n.id + '" class="' + spanCss + ' ">'; // wrapper span
                html += forceOpenNode
                    ? ""
                    : "<span " + auxiliar + ' class="easytree-expander ' + n.iconUrl + ' "></span>';

                //// Esta línea se ocupará solo cuando se requieran imagenes
                //// html += getIconHtml(n);
                html += "<span>&nbsp;&nbsp;&nbsp;</span";
                html += getTitleHtml(n);
        ...
        function getSpanCss(node, lastSibling) {
            ...
            if (node.esCarpeta && _settings.enableDnd) {
        ...
        function getExpCss(node, lastSibling) {
            ...
            if (!hasChildren && node.isLazy) {
                classNode = " menu--item  menu--item__has_sub_menu ";
                exp = "c"; //MFP 08/03/2017 -> menu--item  menu--item__has_sub_menu
            } else if (!hasChildren) { // not children
                classNode = " menu--item";
                exp = "n"; //MFP 08/03/2017 ->menu--item
            ...
            return " easytree-exp-" + exp + classNode;
        }
        ...
        function getIconHtml(node) {
            ...
            if (node.uiIcon) {
                return '<span class="' +
                    classIco +
                    " easytree-custom-icon ui-icon " +
                    node.uiIcon +
                    ' menu--icon"></span>'; //MFP 08/03/2017  ->menu--icon
            }
            if (node.iconUrl) {
                return '<span class="' +
                    classIco +
                    '><img src="' +
                    node.iconUrl +
                    '" /></span>'; //MFP 08/03/2017  ->menu--icon
            }

            return '<span class="easytree-icon ' + classIco + '"></span>'; //MFP 08/03/2017  ->menu--icon
        }
        ...
        function getTitleHtml(node) {
            ...
            var classLink = ""; // MFP 09-03-2017
            if (node.textCss) {
                titleCss += " " + node.textCss;
            }
            var tooltip = node.tooltip ? 'title="' + node.tooltip + '"' : "";
            var MouseHover = ' style="color:white;font-size:14px;position: relative;top:0px;" ';
            html += "<span " + MouseHover + " " + tooltip + ' class="' + titleCss + '">';
            ...
            classLabel = ""; // MFP 09-03-2017
            html += "<span " + ' class="' + classLabel + '" ' + MouseHover + ">" + node.text + "</span>";
            if (node.href) {
                html += "</a>";
            }
            html +=
        ...
        function renderNode(node, openOrClose) {
            ...
            if (expClassStart > -1) { // change arrow/expander class
                ...
                var expClass = expClassEnd > -1
                    ? classes.substring(expClassStart, expClassEnd)
                    : classes.substring(expClassStart);
        ...
        function getMinifiedJson(nodes) { // to increase the chance it can be stored in a 4kb cookie
            var j = JSON.stringify
                ...
                : "Please import json2.js"; // for IE6/7 please import json2.js
            ...
            while (j.indexOf('"esCarpeta":false,') > -1) {
                j = j.replace('"esCarpeta":false,', "");
            }
        ...
        function convertHtmlToNode(element) {
            ...
            node.esCarpeta = $el.hasClass("esCarpeta");
            $el.removeClass("esCarpeta");
